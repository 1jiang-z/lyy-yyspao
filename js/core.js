...
autoSendInterval: 5,
        ...
        // 留言墙设置
        messageWallMinCount: 3,
        messageWallMaxCount: 6,
        // 主页绑定会话开关（默认关闭）
...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    if (msg.type === 'system') {
        ...
        // 对方撤回并修改的系统消息：灰色样式，可点击跳转
        if (msg.subType === 'recall-edit') {
            systemMsgDiv.className = 'system-message recall-edit-tip';
            systemMsgDiv.dataset.targetId = msg.targetMsgId || '';
            systemMsgDiv.innerHTML = `<span class="recall-edit-text">${msg.text}</span>`;
            systemMsgDiv.style.cursor = 'pointer';
            systemMsgDiv.title = '点击查看修改后的消息';
        } else {
    ...
    let content = isXhsCard ? (msg.text || '') : (msg.text ? `<div>${msg.text.replace(/\n/g, '<br>')}</div>` : '');
    if (isVoice) {
        // 微信风格语音消息
    ...
    // 已撤回的消息：替换内容为撤回提示
    if (msg._recalled) {
        var partnerName = (settings && settings.partnerName) ? settings.partnerName : '对方';
        var recallTip = msg.sender === 'user' ? '你撤回了一条消息' : partnerName + ' 撤回了一条消息';
        messageHTML = `<div style="color: #999; font-size: 13px; padding: 4px 0;">${recallTip}</div>`;
    }

    const messageDiv = document.createElement('div');
    var recalledClass = msg._recalled ? ' message-recalled' : '';
    if (isRedPacket || isImageOnly) {
        messageDiv.className = `message message-${msg.sender === 'user' ? 'sent' : 'received'} message-image-bubble-none${recalledClass}`;
    } else if (isVoice) {
        messageDiv.className = `message message-${msg.sender === 'user' ? 'sent' : 'received'} ${settings.bubbleStyle}${recalledClass}`;
        messageDiv.style.padding = '0';
    } else if (isXhsCard) {
        messageDiv.className = `message message-${msg.sender === 'user' ? 'sent' : 'received'} xhs-message-wrapper${recalledClass}`;
    } else {
        messageDiv.className = `message message-${msg.sender === 'user' ? 'sent' : 'received'} ${settings.bubbleStyle}${recalledClass}`;
    }
    ...
    // 对方的消息：模拟对方撤回并修改
    if (msg.sender !== 'user' && msg.type === 'normal' && msg.text && !msg._recalled && !msg._editedFrom) {
        actionsHTML += `<button class="meta-action-btn partner-recall-btn" title="模拟他撤回并修改"><i class="fas fa-rotate-left"></i></button>`;
    }
...
function renderMessages(preserveScroll = false) {
    ...
    if (typeof initXhsCardSliders === 'function') setTimeout(initXhsCardSliders, 0);
...
const addMessage = (message) => {
    ...
    if (message.xhsCard === true && typeof initXhsCardSliders === 'function') setTimeout(initXhsCardSliders, 0);
...
if (!isBatchMode && type === 'normal') {
    ...
    window._pendingReplyTimer = null;
        ...
        window.simulateReply = function() {
            // 防重复：本次回复周期内语音消息只发送一次
            let _voiceSentInThisCycle = false;
                    ...
                    if (i === replyCount - 1) {
                        ...
                        if (shouldSendVoice) {
                            ...
                            setTimeout(() => {
                                ...
                                const doAddVoice = function() {
                                    if (voiceAdded || _voiceSentInThisCycle) return;
                                    voiceAdded = true;
                                    _voiceSentInThisCycle = true;
                                    addMessage(voiceMsg);
                                    playSound('message');
                                };
                                if (randomVoice.audioUrl) {
                                    try {
                                        ...
                                        tmpA.addEventListener('error', () => { doAddVoice(); });
                                        setTimeout(() => { doAddVoice(); }, 2000);
                                    } catch(e) { doAddVoice(); }
                                } else {
                                    doAddVoice();
                                }
...
function showModal(modalElement, focusElement = null) {
        ...
        function exportChatHistory() {
            ...
            overlay.innerHTML = `
                <div style="background:var(--secondary-bg);border-radius:20px;padding:24px;width:88%;max-width:360px;max-height:80vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,0.4);animation:modalContentSlideIn 0.3s ease forwards;">
                    ...
                    <div style="font-size:12px;color:var(--text-secondary);margin-bottom:16px;">勾选需要导出的数据模块，换浏览器后导入即可恢复</div>
                    
                    <div style="display:flex;flex-direction:column;gap:12px;margin-bottom:20px;">
                        <!-- 主页功能 -->
                        <div>
                            <div style="font-size:12px;font-weight:600;color:var(--text-secondary);margin-bottom:8px;padding-left:4px;">主页功能</div>
                            <div style="display:flex;flex-direction:column;gap:6px;">
                                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);transition:border-color 0.2s;">
                                ...
                                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);transition:border-color 0.2s;">
                                    <input type="checkbox" id="_exp_accounting" style="accent-color:var(--accent-color);width:15px;height:15px;">
                                    <i class="fas fa-coins" style="color:var(--accent-color);width:16px;text-align:center;"></i>
                                    <span>恋爱记账</span>
                                </label>
                                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);transition:border-color 0.2s;">
                                    <input type="checkbox" id="_exp_mood" style="accent-color:var(--accent-color);width:15px;height:15px;">
                                    <i class="fas fa-heart" style="color:var(--accent-color);width:16px;text-align:center;"></i>
                                    <span>心情日历</span>
                                </label>
                                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);transition:border-color 0.2s;">
                                    <input type="checkbox" id="_exp_moments" style="accent-color:var(--accent-color);width:15px;height:15px;">
                                    <i class="fas fa-images" style="color:var(--accent-color);width:16px;text-align:center;"></i>
                                    <span>朋友圈</span>
                                </label>
                            </div>
                        </div>
                        ...
                        <!-- 字卡与互动 -->
                        <div>
                            <div style="font-size:12px;font-weight:600;color:var(--text-secondary);margin-bottom:8px;padding-left:4px;">字卡与互动</div>
                            <div style="display:flex;flex-direction:column;gap:6px;">
                                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);transition:border-color 0.2s;">
                                ...
                                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);transition:border-color 0.2s;">
                                    <input type="checkbox" id="_exp_touch" style="accent-color:var(--accent-color);width:15px;height:15px;">
                                    <i class="fas fa-hand-pointer" style="color:var(--accent-color);width:16px;text-align:center;"></i>
                                    <span>戳戳梦角</span>
                                </label>
                                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);transition:border-color 0.2s;">
                                    <input type="checkbox" id="_exp_invitation" style="accent-color:var(--accent-color);width:15px;height:15px;">
                                    <i class="fas fa-heart" style="color:var(--accent-color);width:16px;text-align:center;"></i>
                                    <span>邀请功能</span>
                                </label>
                                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);transition:border-color 0.2s;">
                        ...
                        <!-- 高级功能 -->
                        <div>
                            <div style="font-size:12px;font-weight:600;color:var(--text-secondary);margin-bottom:8px;padding-left:4px;">高级功能</div>
                            <div style="display:flex;flex-direction:column;gap:6px;">
                                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);transition:border-color 0.2s;">
                                    <input type="checkbox" id="_exp_bookshelf" style="accent-color:var(--accent-color);width:15px;height:15px;">
                                    <i class="fas fa-book-open" style="color:var(--accent-color);width:16px;text-align:center;"></i>
                                    <span>书架与问卷</span>
                                </label>
                                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);transition:border-color 0.2s;">
                                    <input type="checkbox" id="_exp_map" style="accent-color:var(--accent-color);width:15px;height:15px;">
                                    <i class="fas fa-map-marker-alt" style="color:var(--accent-color);width:16px;text-align:center;"></i>
                                    <span>地图标记</span>
                                </label>
                                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);transition:border-color 0.2s;">
                                    <input type="checkbox" id="_exp_messagewall" style="accent-color:var(--accent-color);width:15px;height:15px;">
                                    <i class="fas fa-wall" style="color:var(--accent-color);width:16px;text-align:center;"></i>
                                    <span>留言墙</span>
                                </label>
                                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);transition:border-color 0.2s;">
                                    <input type="checkbox" id="_exp_music" style="accent-color:var(--accent-color);width:15px;height:15px;">
                                ...
                                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);transition:border-color 0.2s;">
                                    <input type="checkbox" id="_exp_pet" style="accent-color:var(--accent-color);width:15px;height:15px;">
            ...
            if (_expConfirmBtn) _expConfirmBtn.onclick = async function() {
                const checks = {
                    msgs: !!document.getElementById('_exp_msgs')?.checked,
                    settings: !!document.getElementById('_exp_settings')?.checked,
                    replies: !!document.getElementById('_exp_replies')?.checked,
                    envelope: !!document.getElementById('_exp_envelope')?.checked,
                    themes: !!document.getElementById('_exp_themes')?.checked,
                    diary: !!document.getElementById('_exp_diary')?.checked,
                    accounting: !!document.getElementById('_exp_accounting')?.checked,
                    mood: !!document.getElementById('_exp_mood')?.checked,
                    moments: !!document.getElementById('_exp_moments')?.checked,
                    touch: !!document.getElementById('_exp_touch')?.checked,
                    invitation: !!document.getElementById('_exp_invitation')?.checked,
                    bookshelf: !!document.getElementById('_exp_bookshelf')?.checked,
                    map: !!document.getElementById('_exp_map')?.checked,
                    messagewall: !!document.getElementById('_exp_messagewall')?.checked,
                    music: !!document.getElementById('_exp_music')?.checked,
                    pet: !!document.getElementById('_exp_pet')?.checked
                };
                
                const hasAny = Object.values(checks).some(v => v);
                if (!hasAny) {
                ...
                try {
                    ...
                    if (checks.settings) {
                        ...
                        try { exportObj.dgCustomData = JSON.parse(localStorage.getItem('dg_custom_data') || 'null'); } catch(e) {}
                        try { exportObj.dgStatusPool = JSON.parse(localStorage.getItem('dg_status_pool') || 'null'); } catch(e) {}
                        try {
                            const cw = {};
                            Object.keys(localStorage).forEach(kk => {
                                if (kk && kk.startsWith('customWeather_')) cw[kk] = localStorage.getItem(kk);
                            });
                            exportObj.customWeatherMap = cw;
                        } catch(e) {}
                    ...
                    if (typeof localforage !== 'undefined') {
                        ...
                        for (const key of keys) {
                            ...
                            // 心情日历
                            if (checks.mood && (key.includes('moodCalendar') || key.includes('customMoodOptions') || key.includes('moodTrash'))) {
                                include = true;
                            }
                            // 邀请功能
                            if (checks.invitation && key.startsWith('invitation_')) {
                                include = true;
                            }
                            // 书架问卷（注意 userBookshelf 是大写 B，readerWindowState 没有前缀）
                            if (checks.bookshelf && (key.includes('dreamQuestionnaires') || keyLower.includes('bookshelf') || key.includes('readerWindow'))) {
                                include = true;
                            }
                            // 地图
                            if (checks.map && (key.includes('mapData') || key.includes('MAP_'))) {
                                include = true;
                            }
                            // 留言墙（key 是 messageWallData）
                            if (checks.messagewall && keyLower.includes('messagewall')) {
                                include = true;
                            }
                            // 音乐（key 是 playerCover, customSongs）
                            if (checks.music && (key.includes('playerCover') || key.includes('customSongs') || keyLower.includes('music'))) {
                                include = true;
                            }
                            // 日记
                            if (checks.diary && keyLower.includes('diary')) {
                                include = true;
                            }
                            // 记账
                            if (checks.accounting && keyLower.includes('accounting')) {
                                include = true;
                            }
                            // 朋友圈
                            if (checks.moments && keyLower.includes('moments')) {
                                include = true;
                            }
                    ...
                    if (Object.keys(lfData).length > 0) {
                        exportObj.localforageData = lfData;
                    }
                    
                    // 从 localStorage 读取数据（戳戳、宠物等存在 localStorage 的功能）
                    const lsData = {};
                    const lsKeysToExport = [];
                    
                    // 戳戳
                    if (checks.touch) {
                        lsKeysToExport.push('touch_data');
                    }
                    // 宠物
                    if (checks.pet) {
                        lsKeysToExport.push('luelue_pet_game');
                    }
                    
                    if (lsKeysToExport.length > 0) {
                        for (const k of lsKeysToExport) {
                            try {
                    ...
                    if (Object.keys(lsData).length > 0) {
                        exportObj.localStorageData = lsData;
                    }
                    
                    const parts = exportObj.exportModules.join('+');
                    const dataStr = JSON.stringify(exportObj, null, 2);
                    const fileName = `chatapp-backup-${parts}-${new Date().toISOString().slice(0,10)}.json`;
        ...
        function importChatHistory(file) {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    const hasLocalforage = importedData.localforageData && typeof importedData.localforageData === 'object' && Object.keys(importedData.localforageData).length > 0;
                    const hasLocalStorage = importedData.localStorageData && typeof importedData.localStorageData === 'object' && Object.keys(importedData.localStorageData).length > 0;

                    if (!hasMessages && !hasSettings && !hasReplies && !hasEnvelope && !hasThemes && !hasLocalforage && !hasLocalStorage) {
                        throw new Error('无效的备份文件（未检测到可识别的数据模块）');
                    }
                    
                    // 检测 localforage 里有哪些功能
                    let lfInfo = {
                        mood: false, touch: false, invitation: false, bookshelf: false,
                        map: false, messagewall: false, music: false, pet: false,
                        diary: false, accounting: false, moments: false
                    };
                    if (hasLocalforage) {
                        const keys = Object.keys(importedData.localforageData);
                        lfInfo.mood = keys.some(k => k.includes('moodCalendar') || k.includes('customMoodOptions'));
                        lfInfo.invitation = keys.some(k => k.startsWith('invitation_'));
                        lfInfo.bookshelf = keys.some(k => k.includes('dreamQuestionnaires') || k.toLowerCase().includes('bookshelf') || k.includes('readerWindow'));
                        lfInfo.map = keys.some(k => k.includes('mapData') || k.includes('MAP_'));
                        lfInfo.messagewall = keys.some(k => k.toLowerCase().includes('messagewall'));
                        lfInfo.music = keys.some(k => k.includes('playerCover') || k.includes('customSongs') || k.toLowerCase().includes('music'));
                        lfInfo.diary = keys.some(k => k.toLowerCase().includes('diary'));
                        lfInfo.accounting = keys.some(k => k.toLowerCase().includes('accounting'));
                        lfInfo.moments = keys.some(k => k.toLowerCase().includes('moments'));
                    }
                    
                    // 检测 localStorage 里有哪些功能
                    let lsInfo = {
                        touch: false, pet: false
                    };
                    if (hasLocalStorage) {
                        const lsKeys = Object.keys(importedData.localStorageData);
                        lsInfo.touch = lsKeys.some(k => k === 'touch_data');
                        lsInfo.pet = lsKeys.some(k => k === 'luelue_pet_game');
                    }
                    
                    // 合并功能检测结果（戳戳和宠物可能在 localforage 或 localStorage 中）
                    const featInfo = {
                        mood: lfInfo.mood,
                        touch: lfInfo.touch || lsInfo.touch,
                        invitation: lfInfo.invitation,
                        bookshelf: lfInfo.bookshelf,
                        map: lfInfo.map,
                        messagewall: lfInfo.messagewall,
                        music: lfInfo.music,
                        pet: (lfInfo.pet || false) || lsInfo.pet,
                        diary: lfInfo.diary,
                        accounting: lfInfo.accounting,
                        moments: lfInfo.moments
                    };
                    const featCount = Object.values(featInfo).filter(v => v).length;
                    ...
                    const makeRow = (id, icon, label, sublabel, available, checked) => {
                        if (!available) return '';
                        return `<label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);font-size:13px;color:var(--text-primary);">
                    ...
                    overlay.innerHTML = `
                        <div style="background:var(--secondary-bg);border-radius:20px;padding:24px;width:88%;max-width:360px;max-height:80vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,0.4);animation:modalContentSlideIn 0.3s ease forwards;">
                            ...
                            <div style="display:flex;flex-direction:column;gap:12px;margin-bottom:20px;">
                                <div>
                                    <div style="font-size:12px;font-weight:600;color:var(--text-secondary);margin-bottom:8px;padding-left:4px;">基础数据</div>
                                    <div style="display:flex;flex-direction:column;gap:6px;">
                                ...
                                ${featCount > 0 ? `
                                <div>
                                    <div style="font-size:12px;font-weight:600;color:var(--text-secondary);margin-bottom:8px;padding-left:4px;">功能数据（${featCount}项）</div>
                                    <div style="display:flex;flex-direction:column;gap:6px;">
                                        ${makeRow('_imp_mood', 'fas fa-heart', '心情日历', '', featInfo.mood, true)}
                                        ${makeRow('_imp_touch', 'fas fa-hand-pointer', '戳戳梦角', '', featInfo.touch, true)}
                                        ${makeRow('_imp_invitation', 'fas fa-heart-broken', '邀请功能', '', featInfo.invitation, true)}
                                        ${makeRow('_imp_bookshelf', 'fas fa-book-open', '书架与问卷', '', featInfo.bookshelf, true)}
                                        ${makeRow('_imp_diary', 'fas fa-book', '日记本', '', featInfo.diary, false)}
                                        ${makeRow('_imp_accounting', 'fas fa-coins', '恋爱记账', '', featInfo.accounting, false)}
                                        ${makeRow('_imp_moments', 'fas fa-images', '朋友圈', '', featInfo.moments, false)}
                                        ${makeRow('_imp_map', 'fas fa-map-marker-alt', '地图标记', '', featInfo.map, false)}
                                        ${makeRow('_imp_messagewall', 'fas fa-wall', '留言墙', '', featInfo.messagewall, false)}
                                        ${makeRow('_imp_music', 'fas fa-music', '音乐歌单', '', featInfo.music, false)}
                                        ${makeRow('_imp_pet', 'fas fa-paw', '宠物养成', '', featInfo.pet, false)}
                                    </div>
                                </div>
                                ` : ''}
                            </div>
                    ...
                    if (_impConfirmBtn) _impConfirmBtn.onclick = async function() {
                        ...
                        const doMood       = featInfo.mood       && !!document.getElementById('_imp_mood')?.checked;
                        const doTouch      = featInfo.touch      && !!document.getElementById('_imp_touch')?.checked;
                        const doInvitation = featInfo.invitation && !!document.getElementById('_imp_invitation')?.checked;
                        const doBookshelf  = featInfo.bookshelf  && !!document.getElementById('_imp_bookshelf')?.checked;
                        const doDiary      = featInfo.diary      && !!document.getElementById('_imp_diary')?.checked;
                        const doAccounting = featInfo.accounting && !!document.getElementById('_imp_accounting')?.checked;
                        const doMoments    = featInfo.moments    && !!document.getElementById('_imp_moments')?.checked;
                        const doMap        = featInfo.map        && !!document.getElementById('_imp_map')?.checked;
                        const doMsgWall    = featInfo.messagewall && !!document.getElementById('_imp_messagewall')?.checked;
                        const doMusic      = featInfo.music      && !!document.getElementById('_imp_music')?.checked;
                        const doPet        = featInfo.pet        && !!document.getElementById('_imp_pet')?.checked;
                        
                        const hasAnyFeat = doMood || doTouch || doInvitation || doBookshelf || doDiary || doAccounting || doMoments || doMap || doMsgWall || doMusic || doPet;

                        if (!doMsgs && !doSettings && !doReplies && !doEnvelope && !doThemes && !hasAnyFeat) {
                        ...
                        // 恢复 localforage 数据（按选择的功能过滤）
                        if (hasAnyFeat && importedData.localforageData) {
                            if (typeof localforage !== 'undefined') {
                                const lfKeys = Object.keys(importedData.localforageData);
                                for (const key of lfKeys) {
                                    let include = false;
                                    const keyLower = key.toLowerCase();
                                    if (doMood && (key.includes('moodCalendar') || key.includes('customMoodOptions') || key.includes('moodTrash'))) include = true;
                                    if (doInvitation && key.startsWith('invitation_')) include = true;
                                    if (doBookshelf && (key.includes('dreamQuestionnaires') || keyLower.includes('bookshelf') || key.includes('readerWindow'))) include = true;
                                    if (doDiary && keyLower.includes('diary')) include = true;
                                    if (doAccounting && keyLower.includes('accounting')) include = true;
                                    if (doMoments && keyLower.includes('moments')) include = true;
                                    if (doMap && (key.includes('mapData') || key.includes('MAP_'))) include = true;
                                    if (doMsgWall && keyLower.includes('messagewall')) include = true;
                                    if (doMusic && (key.includes('playerCover') || key.includes('customSongs') || keyLower.includes('music'))) include = true;
                                    
                                    if (include) {
                                        try {
                                            let targetKey = key;
                                            if (importedData.sessionId && SESSION_ID && importedData.sessionId !== SESSION_ID) {
                                                targetKey = key.replace(importedData.sessionId, SESSION_ID);
                                            }
                                            await localforage.setItem(targetKey, importedData.localforageData[key]);
                                        } catch(e) {
                                            console.warn('恢复 localforage 数据失败:', key, e);
                                        }
                                    }
                                }
                            }
                        ...
                        // 恢复 localStorage 数据（戳戳、宠物等）
                        if (hasAnyFeat && importedData.localStorageData) {
                            const lsKeys = Object.keys(importedData.localStorageData);
                            for (const key of lsKeys) {
                                let include = false;
                                if (doTouch && key === 'touch_data') include = true;
                                if (doPet && key === 'luelue_pet_game') include = true;
                                
                                if (include) {
                                    try {
                                        localStorage.setItem(key, importedData.localStorageData[key]);
                                    } catch(e) {
                                        console.warn('恢复 localStorage 数据失败:', key, e);
                                    }
                                }
                            }
                        }
                        ...
                        let parts = [];
                        if (doMsgs) parts.push(`${messages.length}条消息`);
                        if (doSettings) parts.push('设置');
                        if (doMood) parts.push('心情');
                        if (doTouch) parts.push('戳戳');
                        if (doInvitation) parts.push('邀请');
                        if (doBookshelf) parts.push('书架');
                        const count = parts.length > 0 ? parts.join('、') : '所选数据';
                        showNotification(`成功导入${count}，刷新页面生效`, 'success');
                    };
                } catch (error) {
