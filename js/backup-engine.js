...
(function (global) {
    ...
    function buildModuleSkipPatterns(flags) {
        ...
        if (!flags.inclTarot) p.push('diviHistory', 'customTarotDeck', 'customTarotEnabled', 'tarot_data');
        ...
        // 新增：书架、戳戳、邀请、留言墙、音乐
        if (!flags.inclBookshelf) p.push('dreamQuestionnaires', 'userBookshelf', 'readerWindowState', 'bookshelf');
        if (!flags.inclTouch) p.push('touch_data', 'touchData');
        if (!flags.inclInvitation) p.push('invitation_');
        if (!flags.inclMessageWall) p.push('messageWallData', 'messageWall');
        if (!flags.inclMusic) p.push('playerCover', 'customSongs');
        if (!flags.inclBattleCompanion) p.push('battleCompanionData');
        return p;
    }
    ...
    /**
        ...
        flags = flags || {
            ...
            inclBookshelf: true, inclTouch: true, inclInvitation: true,
            inclMessageWall: true, inclMusic: true, inclBattleCompanion: true
        };
        var lfData = {};
