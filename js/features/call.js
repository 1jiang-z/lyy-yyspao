(function () {
    ...
    function isSoundEnabled() {
        try {
            return window.settings && window.settings.soundEnabled !== false;
        } catch(e) { return true; }
    }

    function getSoundVolume() {
        try {
            return Math.min(1, Math.max(0, window.settings.soundVolume || 0.3));
        } catch(e) { return 0.3; }
    }
    ...
    function startRingTone() {
        ...
        masterGain.gain.value = getSoundVolume() * 1.0;
        masterGain.connect(ctx.destination);

        // 节奏更紧凑的铃声，循环间隔短，更连续
        function playRingChime(startTime) {
            const notes = [
                { freq: 587.33, dur: 0.18, delay: 0 },      // D5
                { freq: 783.99, dur: 0.18, delay: 0.20 },   // G5
                { freq: 659.25, dur: 0.22, delay: 0.40 },   // E5
                { freq: 880.00, dur: 0.36, delay: 0.64 },   // A5（长音）
            ];
            ...
            notes.forEach((note) => {
                ...
                g1.gain.value = 1.0;     // 三角波为主
                g2.gain.value = 0.25;    // 方波做泛音，增加穿透力

                const t0 = startTime + note.delay;
                const env = ctx.createGain();
                env.gain.setValueAtTime(0, t0);
                env.gain.linearRampToValueAtTime(0.75, t0 + 0.01);    // 极快起音
                env.gain.setValueAtTime(0.72, t0 + note.dur * 0.7);
                env.gain.linearRampToValueAtTime(0, t0 + note.dur);   // 快速收尾
                ...
                osc1.start(t0);
                osc2.start(t0);
                osc1.stop(t0 + note.dur + 0.01);
                osc2.stop(t0 + note.dur + 0.01);
            });
        }

        // 循环更紧凑：响约 1s + 停约 0.5s，更连续
        function scheduleLoop() {
            if (!S.audioCtx) return;
            playRingChime(S.audioCtx.currentTime);
            S.ringTimer = setTimeout(scheduleLoop, 1500);
        }

        S.ringGain = masterGain;
        scheduleLoop();
    }
    ...
    function startBusyTone() {
        ...
        const masterGain = ctx.createGain();
        masterGain.gain.value = getSoundVolume() * 0.25;
        masterGain.connect(ctx.destination);
        ...
        function playBeep() {
            ...
            gain.gain.setValueAtTime(0, S.audioCtx.currentTime);
            gain.gain.linearRampToValueAtTime(0.8, S.audioCtx.currentTime + 0.02);
            gain.gain.setValueAtTime(0.8, S.audioCtx.currentTime + 0.35);
            gain.gain.linearRampToValueAtTime(0, S.audioCtx.currentTime + 0.4);
            osc.connect(gain);
            gain.connect(masterGain);
            osc.start();
            osc.stop(S.audioCtx.currentTime + 0.42);
        }

        // 每 0.8s 响一声（0.4s 响 + 0.4s 停）
        playBeep();
        S.busyTimer = setInterval(playBeep, 800);
        S.busyGain = masterGain;
    }
    ...
    function playConnectTone() {
        ...
        gain.gain.linearRampToValueAtTime(getSoundVolume() * 0.3, now + 0.03);
        gain.gain.setValueAtTime(getSoundVolume() * 0.3, now + 0.15);
    ...
    function playHangupTone() {
        ...
        gain.gain.setValueAtTime(0, now);
        gain.gain.linearRampToValueAtTime(getSoundVolume() * 0.35, now + 0.03);
        gain.gain.setValueAtTime(getSoundVolume() * 0.35, now + 0.2);
