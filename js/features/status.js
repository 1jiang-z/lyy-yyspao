...
(function() {
    ...
    // ===== 绑定入口 =====
    function bindEntry() {
        const entry = document.getElementById('status-function');
        if (entry) {
            entry.addEventListener('click', () => {
                ...
                } else if (advancedModal && typeof window.homeHideModal === 'function') {
                    window.homeHideModal(advancedModal);
                }
                // 延迟打开状态模态框，等高级功能关闭动画完成
                setTimeout(() => {
                    open();
                }, 300);
            });
    ...
    function checkAutoAdd() {
        ...
        // 他改变自己某个状态的数值（每次打开20%概率）
        if (Math.random() < 0.20 && stateData.hisStates.length > 0) {
    ...
    function render() {
        ...
        container.innerHTML = `
            ...
            <div class="status-tabs">
                <button class="status-tab ${currentTab === 'his' ? 'active' : ''}" onclick="window.StatusApp.switchTab('his')">
                    <i class="fas fa-mars"></i> ${partnerName}的状态
                </button>
                <button class="status-tab ${currentTab === 'my' ? 'active' : ''}" onclick="window.StatusApp.switchTab('my')">
                    <i class="fas fa-venus"></i> 我的状态
                </button>
                <button class="status-tab ${currentTab === 'records' ? 'active' : ''}" onclick="window.StatusApp.switchTab('records')">
            ...
            <div class="status-panel" id="status-panel-his" style="display:${currentTab === 'his' ? 'block' : 'none'};">
                ...
                <div class="status-manage-row">
                    <button class="status-manage-btn" onclick="window.StatusApp.showManage('his')">
            ...
            <div class="status-panel" id="status-panel-my" style="display:${currentTab === 'my' ? 'block' : 'none'};">
                ...
                <div class="status-manage-row">
                    <button class="status-manage-btn" onclick="window.StatusApp.showManage('my')">
                        <i class="fas fa-book"></i> 我的状态字卡库
                    </button>
                    <button class="status-add-main-btn" onclick="window.StatusApp.addMyState()">
            ...
            <div class="status-panel" id="status-panel-library" style="display:none;">
                <div class="status-manage-header">
                    <button class="status-back-btn" onclick="window.StatusApp.hideManage()">
                ...
                <div class="status-lib-add">
                    ...
                    <button class="status-lib-add-btn" onclick="window.StatusApp.addToLibrary()">
        ...
        return `
            <div class="status-list">
                ${states.map((state, index) => `
                    <div class="status-item ${state.heAdded ? 'he-added' : ''}" data-id="${state.id}" data-side="${side}">
                        ...
                        <div class="status-actions">
                            ${canEdit ? `
                                <button class="status-btn minus" onclick="window.StatusApp.decrease('${side}', ${index})">
                                    <i class="fas fa-minus"></i>
                                </button>
                                <button class="status-btn plus" onclick="window.StatusApp.increase('${side}', ${index})">
                                    <i class="fas fa-plus"></i>
                                </button>
                            ` : `
                                <span class="status-cant-edit">只有他能改哦</span>
                            `}
                            <button class="status-btn edit" onclick="window.StatusApp.editState('${side}', ${index})">
                                <i class="fas fa-pen"></i>
                            </button>
                            ${state.custom ? `<button class="status-btn delete" onclick="window.StatusApp.deleteState('${side}', ${index})"><i class="fas fa-trash"></i></button>` : ''}
                        </div>
                    </div>
                `).join('')}
            </div>
    ...
    function renderRecords() {
        ...
        return `
            ...
            ${stateData.records.length > 0 ? `
                <button class="status-clear-records-btn" onclick="window.StatusApp.clearRecords()">
                    ...
                    ${library.map((word, index) => {
                        ...
                        return `
                            <div class="status-lib-item ${isActive ? 'active' : ''}">
                                ...
                                <button class="lib-del-btn" onclick="window.StatusApp.removeFromLibrary(${index})">
