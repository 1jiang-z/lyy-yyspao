...
if (target.classList.contains('delete-btn')) {
    ...
    return;
                ...
                if (target.classList.contains('partner-recall-btn')) {
                    // 模拟对方撤回并修改
                    if (typeof window.doPartnerRecallEdit === 'function') {
                        window.doPartnerRecallEdit(messageId);
                    }
                    return;
                }
...
// ===== 对方撤回并修改功能 =====
(function initPartnerRecallEdit() {
    // 右键菜单：对方消息右键可触发"模拟对方撤回并修改"
    var contextMenu = null;

    function createContextMenu() {
        if (contextMenu) return;
        contextMenu = document.createElement('div');
        contextMenu.id = 'partner-recall-context-menu';
        contextMenu.className = 'partner-recall-context-menu';
        contextMenu.style.display = 'none';
        contextMenu.innerHTML = `
            <div class="ctx-menu-item" data-action="recall-edit">
                <i class="fas fa-undo"></i> 模拟对方撤回并修改
            </div>
        `;
        ...
        contextMenu.addEventListener('click', function(e) {
            ...
            var msgId = Number(contextMenu.dataset.msgId);

            if (action === 'recall-edit') {
                doPartnerRecallEdit(msgId);
            }
    ...
    function bindContextMenu() {
        if (!DOMElements || !DOMElements.chatContainer) return;

        DOMElements.chatContainer.addEventListener('contextmenu', function(e) {
            var wrapper = e.target.closest('.message-wrapper');
            if (!wrapper) return;

            var messageId = Number(wrapper.dataset.id);
            var message = messages.find(m => m.id === messageId);
            if (!message) return;

            // 只对对方的非系统、非通话事件消息显示右键菜单
            if (message.sender !== 'user' && message.type !== 'system' && message.type !== 'call-event' && !message._recalled) {
                e.preventDefault();
    ...
    // 对方撤回并修改的核心逻辑
    function doPartnerRecallEdit(msgId, customNewText) {
        var idx = messages.findIndex(m => m.id === msgId);
        if (idx === -1) return;

        var originalMsg = messages[idx];
        var partnerName = (settings && settings.partnerName) ? settings.partnerName : '对方';

        var newText;
        if (customNewText) {
            newText = customNewText;
        } else {
            // 弹出输入框让用户输入修改后的内容（模拟对方修改）
            var inputText = prompt('请输入对方修改后的消息内容：', originalMsg.text || '');
            if (inputText === null) return; // 取消
            newText = inputText.trim();
            if (!newText) {
                showNotification('内容不能为空', 'warning', 2000);
                return;
            }
        }
        ...
        // 1. 将原消息标记为已撤回（保留原始内容，隐藏显示）
        var recalledMsg = Object.assign({}, originalMsg, {
            id: msgId, // 保持原ID
            _recalled: true,
            _originalText: originalMsg.text,
            _editedTo: newText
        });
        messages[idx] = recalledMsg;

        // 2. 在原消息后面插入系统提示（灰色样式，可点击跳转到修改后的新消息）
        var systemMsgId = 'recall_edit_' + Date.now();
        var newMsgId = Date.now() + 1;
        var systemMsg = {
            id: systemMsgId,
            sender: 'system',
            text: partnerName + '撤回了一条信息，撤回的内容为「' + (originalMsg.text || '') + '」。修改成为「' + newText + '」。',
            timestamp: new Date().toISOString(),
            type: 'system',
            subType: 'recall-edit',
            targetMsgId: newMsgId, // 指向修改后的新消息
            editedText: newText
        };
        messages.splice(idx + 1, 0, systemMsg);
        ...
        var newMsg = {
            ...
            timestamp: new Date().toISOString(),
            type: 'normal',
            _editedFrom: msgId // 标记为从哪条消息修改而来
        };
        messages.splice(idx + 2, 0, newMsg);
        ...
        if (!customNewText) {
            showNotification('对方撤回并修改了消息', 'success', 2000);
        }
    }

    // ===== 梦角自动撤回修改功能 =====
    // 梦角发送的消息有一定概率自动撤回并修改
    var autoRecallProbability = 0.06; // 6% 概率自动撤回修改
    var processedMsgIds = {}; // 已处理过的消息id，避免重复

    // 从字卡库生成新的回复内容
    function generateNewReplyText() {
        try {
            if (typeof customReplies !== 'undefined' && customReplies.length > 0) {
                var pool = customReplies.map(function(r) { return String(r || '').trim(); }).filter(Boolean);
                if (pool.length > 0) {
                    return pool[Math.floor(Math.random() * pool.length)];
                }
            }
        } catch (e) {}
        return '嗯...刚才那句话说得不太对，重新说一下。';
    }

    // 包装 addMessage：对方消息添加后，有概率触发撤回修改
    function wrapAddMessage() {
        if (typeof window.addMessage !== 'function') return;
        if (window._recallAddMessageWrapped) return;

        var originalAddMessage = window.addMessage;
        window._recallAddMessageWrapped = true;

        window.addMessage = function(message) {
            var result = originalAddMessage.call(this, message);

            // 只处理对方的普通文本消息
            if (message && message.sender && message.sender !== 'user' && message.sender !== 'system'
                && message.type === 'normal' && message.text && !message.image && !message._editedFrom) {

                var msgId = message.id;
                // 避免重复处理
                if (processedMsgIds[msgId]) return result;
                processedMsgIds[msgId] = true;

                // 概率触发撤回修改
                if (Math.random() < autoRecallProbability) {
                    // 延迟一段时间后撤回（模拟对方"发出去又觉得不对"的感觉）
                    var delay = 5000 + Math.random() * 10000; // 5-15秒后撤回
                    setTimeout(function() {
                        // 再次确认消息还在且未被撤回
                        var idx = messages.findIndex(function(m) { return m.id === msgId; });
                        if (idx === -1) return;
                        var currentMsg = messages[idx];
                        if (!currentMsg || currentMsg._recalled) return;

                        var newText = generateNewReplyText();
                        if (!newText) return;

                        // 执行撤回修改
                        doPartnerRecallEdit(msgId, newText);
    ...
    // 系统消息点击跳转
    function bindSystemMsgClick() {
        if (!DOMElements || !DOMElements.chatContainer) return;

        DOMElements.chatContainer.addEventListener('click', function(e) {
            var sysMsg = e.target.closest('.recall-edit-tip');
            if (!sysMsg) return;

            var targetId = sysMsg.dataset.targetId;
            if (!targetId) return;

            // 找到目标消息并滚动到它的位置
            var targetWrapper = document.querySelector('.message-wrapper[data-id="' + targetId + '"]');
            if (targetWrapper) {
                targetWrapper.scrollIntoView({ behavior: 'smooth', block: 'center' });
                // 高亮一下
                targetWrapper.style.transition = 'background 0.3s';
                targetWrapper.style.background = 'rgba(var(--accent-color-rgb), 0.15)';
                setTimeout(function() {
                    targetWrapper.style.background = '';
                }, 1500);
            }
        });
    }
    ...
    // 暴露到全局
    window.doPartnerRecallEdit = doPartnerRecallEdit;
