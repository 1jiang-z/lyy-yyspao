    ...
    if (diaryTodos.length === 0) {
        container.innerHTML = `
            <div class="diary-empty">
                ...
                <p>还没有待办事项</p>
                <p style="font-size:12px;opacity:0.5;margin-top:4px;">在下方输入框添加你的第一个待办</p>
            </div>`;
        return;
    }
...
function quickAddTodo() {
    const quickInput = document.getElementById('diary-todo-quick-input');
    ...
    // 使用当前选中的分类
    const category = diaryTodoSubTab !== 'all' ? diaryTodoSubTab : 'other';
...
// 高亮新添加的待办项
function highlightNewTodo(todoId) {
    setTimeout(() => {
        const item = document.querySelector(`.diary-todo-item[data-id="${todoId}"]`);
        if (item) {
            item.classList.add('new-added');
            setTimeout(() => {
...
function initDiaryListeners() {
    ...
    // 快速添加栏 - 按钮点击
    const quickAddBtn = document.getElementById('diary-todo-quick-add-btn');
    if (quickAddBtn && !quickAddBtn.dataset.bound) {
        quickAddBtn.dataset.bound = 'true';
